...
namespace ProyectoAplicadaI.BLL
{
    public class CobrosBLL
    {
        ...
        public static bool Guardar(Cobros cobro)
        {
            ...
            try
            {
                if (contexto.Cobro.Add(cobro) != null)
                {
                    ...
                    foreach (var item in EmpeñosBLL.GetList(x => x.EmpeñoId == cobro.EmpeñoId))
                    {
                    ...
                    foreach (var item in EmpeñosBLL.GetList(x => x.EmpeñoId == cobro.EmpeñoId))
                    {
                        ...
                        {
                            EmpeñosBLL.EliminarParaCobro(cobro.EmpeñoId);
                        }
                    }
                    paso = true;
                }
        ...
        public static bool Modificar(Cobros cobro)
        {
            ...
            try
            {
                Cobros Anterior = BLL.CobrosBLL.Buscar(cobro.CobroId);
                decimal diferencia;
                diferencia = Anterior.Abono + cobro.Abono;
                decimal otradif = Anterior.Abono - cobro.Abono;
                Empeños recibos = EmpeñosBLL.Buscar(cobro.EmpeñoId);
                ...
                EmpeñosBLL.ModificarEspecial(recibos);
                foreach (var item in EmpeñosBLL.GetList(x => x.EmpeñoId == cobro.EmpeñoId))
                {
                    if ((item.MontoTotal + Ganancia(item.Fecha, item.MontoTotal)) - item.Abono == 0)
                    {
